...
public class UntilElementNotEnabled extends WaitKeyword {
	...
	public static WaitKeyword instantiateExternal(KeywordProvider keywordProvider, List<ObjectDef> defs,
			List<String> params) {
		if(isBy(defs.get(0))) {
			return new UntilElementNotEnabled(keywordProvider.driver, castToBy(defs.get(0)), keywordProvider.loggers,  (long)Double.parseDouble(params.get(0)));
		} else {
			return new UntilElementNotEnabled(keywordProvider.driver, castToElem(defs.get(0)), keywordProvider.loggers,  (long)Double.parseDouble(params.get(0)));
		}
	}
