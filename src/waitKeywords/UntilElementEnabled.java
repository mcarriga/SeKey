...
public class UntilElementEnabled extends WaitKeyword {
	...
	public static WaitKeyword instantiateExternal(KeywordProvider keywordProvider, List<ObjectDef> defs,
			List<String> params) {
		if(isBy(defs.get(0))) {
			return new UntilElementEnabled(keywordProvider.driver, castToBy(defs.get(0)), keywordProvider.loggers,  (long)Double.parseDouble(params.get(0)));
		} else {
			return new UntilElementEnabled(keywordProvider.driver, castToElem(defs.get(0)), keywordProvider.loggers,  (long)Double.parseDouble(params.get(0)));
		}
	}
