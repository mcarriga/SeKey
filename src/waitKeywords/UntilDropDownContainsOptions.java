...
public class UntilDropDownContainsOptions extends WaitKeyword {
	...
	public static WaitKeyword instantiateExternal(KeywordProvider keywordProvider, List<ObjectDef> defs,
		...
		if(isBy(defs.get(0))) {
			return new UntilDropDownContainsOptions(keywordProvider.driver, castToBy(defs.get(0)), values, keywordProvider.loggers, time);
		} else {
			return new UntilDropDownContainsOptions(keywordProvider.driver, castToElem(defs.get(0)), values, keywordProvider.loggers, time);
		}
	}
}
