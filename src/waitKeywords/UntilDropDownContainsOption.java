...
public class UntilDropDownContainsOption extends WaitKeyword {
	...
	public static WaitKeyword instantiateExternal(KeywordProvider keywordProvider, List<ObjectDef> defs,
			List<String> params) {
		if(isBy(defs.get(0))) {
			return new UntilDropDownContainsOption(keywordProvider.driver, castToBy(defs.get(0)), params.get(0), keywordProvider.loggers, (long)Double.parseDouble(params.get(1)));
		} else {
			return new UntilDropDownContainsOption(keywordProvider.driver, castToElem(defs.get(0)), params.get(0), keywordProvider.loggers, (long)Double.parseDouble(params.get(1)));
		}
	}
}
