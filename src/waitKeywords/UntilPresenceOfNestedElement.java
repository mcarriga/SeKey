...
public class UntilPresenceOfNestedElement extends WaitKeyword {
	...
	public static WaitKeyword instantiateExternal(KeywordProvider keywordProvider, List<ObjectDef> defs,
			List<String> params) {
		if(isBy(defs.get(0))) {
			return new UntilPresenceOfNestedElement(keywordProvider.driver, castToBy(defs.get(0)), castToBy(defs.get(1)), keywordProvider.loggers, (long)Double.parseDouble(params.get(1)));
		} else {
			return new UntilPresenceOfNestedElement(keywordProvider.driver, castToElem(defs.get(0)), castToBy(defs.get(1)), keywordProvider.loggers, (long)Double.parseDouble(params.get(1)));
		}
	}
}
