...
public class AssertElementNotVisible extends AssertKeyword {
	...
	public static AssertKeyword instantiateExternal(Framework framework, List<ObjectDef> defs,
		...
		if(isBy(defs.get(0))) {
			if(params.size() > 0) {
				return new AssertElementNotVisible(framework.driver, framework.logger, framework.wait, castToBy(defs.get(0)),  (long)Double.parseDouble(params.get(0)));
			} else {
				return new AssertElementNotVisible(framework.driver, framework.logger, framework.wait, castToBy(defs.get(0)), framework.asserter.getDefaultWait());
			}
		} else {
			if(params.size() > 0) {
				return new AssertElementNotVisible(castToElem(defs.get(0)), framework.logger, framework.wait,  (long)Double.parseDouble(params.get(0)));
			} else {
				return new AssertElementNotVisible(castToElem(defs.get(0)), framework.logger, framework.wait, framework.asserter.getDefaultWait());
			}
		}
	}
}
