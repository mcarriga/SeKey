...
public class UntilDropDownItemCountIs extends WaitKeyword {
	...
	public static WaitKeyword instantiateExternal(Framework framework, List<ObjectDef> defs,
			List<String> params) {
		if(isBy(defs.get(0))) {
			return new UntilDropDownItemCountIs(framework.driver, castToBy(defs.get(0)), (int)Double.parseDouble(params.get(0)), framework.logger, (long)Double.parseDouble(params.get(1)));
		} else {
			return new UntilDropDownItemCountIs(framework.driver, castToElem(defs.get(0)), (int)Double.parseDouble(params.get(0)), framework.logger, (long)Double.parseDouble(params.get(1)));
		}
	}
}
