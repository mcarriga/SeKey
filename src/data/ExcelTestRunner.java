...
public class ExcelTestRunner {
	...
	public ExcelTestRunner(ExcelWorkbook workbook, Iterable<String> sheetsToRun) {
		this.sheetsToRun = sheetsToRun;
		this.workbook = workbook;
	}
	
	public ExcelTestRunner(ExcelWorkbook workbook, String[] sheetsToRun) {
		this(workbook, Arrays.asList(sheetsToRun));
	}
	
	public HashMap<ExcelWorksheet, List<ExcelTest>> init() {
		
		HashMap<ExcelWorksheet, List<ExcelTest>> Suite = new HashMap<ExcelWorksheet, List<ExcelTest>>();
		...
		for(ExcelWorksheet sheet : allWorkSheets) {
			List<ExcelTest> testsToRunInWorksheet = getAllValidTestsInRunnerSheet(sheet);
			Suite.put(sheet, testsToRunInWorksheet);
		}
		return Suite;
	}
	
	private List<ExcelTest> getAllValidTestsInRunnerSheet(ExcelWorksheet sheet) {
		
		List<ExcelTest> testsInRunnerSheet = new ArrayList<ExcelTest>();
