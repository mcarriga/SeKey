...
public class SelectCheckbox extends ActionKeyword {
	...
	public static ActionKeyword instantiateExternal(KeywordProvider keywordProvider, List<ObjectDef> defs,
			List<String> params) {
		if(isBy(defs.get(0))) {
			return new SelectCheckbox(keywordProvider.driver, keywordProvider.loggers, keywordProvider.waits, castToBy(defs.get(0)));
		} else {
			return new SelectCheckbox(castToElem(defs.get(0)), keywordProvider.loggers, keywordProvider.waits);
		}
	}
