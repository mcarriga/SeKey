...
public class SelectRadioOptionByText extends ActionKeyword {
	...
	public static ActionKeyword instantiateExternal(Framework framework, List<ObjectDef> defs,
			List<String> params) {
		if(isBy(defs.get(0))) {
			return new SelectRadioOptionByText(framework.driver, castToBy(defs.get(0)), params.get(0), framework.logger, framework.wait);
		} else {
			return new SelectRadioOptionByText(castToElem(defs.get(0)), params.get(0), framework.logger, framework.wait);
		}
	}
