...
public class AssertDropDownContainsOption extends AssertKeyword {
	...
	public static AssertKeyword instantiateExternal(KeywordProvider keywordProvider, List<ObjectDef> defs,
		...
		if(isBy(defs.get(0))) {
			if(params.size() > 1) {
				return new AssertDropDownContainsOption(keywordProvider.driver, castToBy(defs.get(0)), params.get(0), keywordProvider.loggers, keywordProvider.waits,  (long)Double.parseDouble(params.get(1)));
			} else {
				return new AssertDropDownContainsOption(keywordProvider.driver, castToBy(defs.get(0)), params.get(0), keywordProvider.loggers, keywordProvider.waits, keywordProvider.asserts.getDefaultWait());
			}
		} else {
			if(params.size() > 1) {
				return new AssertDropDownContainsOption(castToElem(defs.get(0)), params.get(0), keywordProvider.loggers, keywordProvider.waits,  (long)Double.parseDouble(params.get(1)));
			} else {
				return new AssertDropDownContainsOption(castToElem(defs.get(0)), params.get(0), keywordProvider.loggers, keywordProvider.waits, keywordProvider.asserts.getDefaultWait());
			}
		}
	}
