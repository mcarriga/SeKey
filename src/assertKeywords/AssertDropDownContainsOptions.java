...
public class AssertDropDownContainsOptions extends AssertKeyword {
	...
	public static AssertKeyword instantiateExternal(KeywordProvider keywordProvider, List<ObjectDef> defs,
			List<String> params) {
		if(isBy(defs.get(0))) {
			return new AssertDropDownContainsOptions(keywordProvider.driver, castToBy(defs.get(0)), params, keywordProvider.loggers, keywordProvider.waits, keywordProvider.asserts.getDefaultWait());
		} else {
			return new AssertDropDownContainsOptions(castToElem(defs.get(0)), params, keywordProvider.loggers, keywordProvider.waits, keywordProvider.asserts.getDefaultWait());
		}
	}
}
