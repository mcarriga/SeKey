...
public class AssertElementNotSelected extends AssertKeyword {
	...
	public static AssertKeyword instantiateExternal(KeywordProvider keywordProvider, List<ObjectDef> defs,
		...
		if(isBy(defs.get(0))) {
			if(params.size() > 0) {
				return new AssertElementNotSelected(keywordProvider.driver, keywordProvider.loggers, keywordProvider.waits, castToBy(defs.get(0)),  (long)Double.parseDouble(params.get(0)));
			} else {
				return new AssertElementNotSelected(keywordProvider.driver, keywordProvider.loggers, keywordProvider.waits, castToBy(defs.get(0)), keywordProvider.asserts.getDefaultWait());
			}
		} else {
			if(params.size() > 0) {
				return new AssertElementNotSelected(castToElem(defs.get(0)), keywordProvider.loggers, keywordProvider.waits,  (long)Double.parseDouble(params.get(0)));
			} else {
				return new AssertElementNotSelected(castToElem(defs.get(0)), keywordProvider.loggers, keywordProvider.waits, keywordProvider.asserts.getDefaultWait());
			}
		}
	}
